...
class KalibrierTabelle:
    ...
    def delete_point(self, index):
        """Löscht einen Punkt anhand des Indexes."""
        try:
            index = int(index)
            v, p = self.lookup_table.pop(index)
            self.save()
            print(f"[{self.filename}] Punkt gelöscht: {v:.2f} V → {p:.2f} W")
        except Exception as e:
            print("Fehler beim Löschen:", e)

    def change_point(self, index, new_voltage, new_power):
        """Ändert einen bestehenden Kalibrierpunkt."""
        try:
            index = int(index)
            new_voltage = float(new_voltage)
            new_power = float(new_power)
            old = self.lookup_table[index]
            self.lookup_table[index] = (new_voltage, new_power)
            self.lookup_table.sort()
            self.save()
            print(f"[{self.filename}] Punkt geändert: {old} → ({new_voltage:.2f} V → {new_power:.2f} W)")
        except Exception as e:
            print("Fehler beim Ändern:", e)
    ...
    def show_table(self):
        ...
        if not self.lookup_table:
            ...
            return
        for i, (v, p) in enumerate(self.lookup_table):
            print(f"  [{i}] {v:>6.2f} V → {p:>6.2f} W")
